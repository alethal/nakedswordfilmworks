...
var DashPlayer = function() {
	...
	this.MAX_SOURCE_BUFFER_TIME = 5; // Maximum amount of time in seconds we will hold in the MSE buffer
...
//		player.stream.seek(player.videoElement.currentTime);
	...
	this._onInitialized = function() { // called after init once we have loaded audio & video into MSE
		if (player.startTime) {
			player.videoElement.currentTime = player.startTime;
		}
...
//console.log('ct:'+stream.player.videoElement.currentTime+' vd:'+stream.duration);
			if (stream.duration - stream.player.videoElement.currentTime < lastPeriod.segmentDuration*2) {
...
//console.log('SSI:'+streamSegIndex+' MSI:'+mediaSegIndex+' PI:'+i+' NPS:'+stream.periods[i].numSegments);
		return {periodIndex:i,mediaSegmentIndex:(mediaSegIndex+stream.periods[i].mediaSegmentOffset)};
	}
...
//console.log('append decoder check '+(adaptationSet.isVideo?'Video':'Audio')+' BL:'+adaptationSet.buffer.length+' I:'+adaptationSet.stream.initializing+' HRM:'+adaptationSet.stream.hardResetMode);
			if (streamComponent.sourceBufferFlushCount > 10) {
				...
				if (streamComponent.stream.player.videoElement.currentTime-currentPeriod.segmentDuration*3 > 0) { // may have seeked to 0
					streamComponent.sourceBuffer.remove(0,streamComponent.stream.player.videoElement.currentTime-currentPeriod.segmentDuration*3);
				}
			}
...
//console.log('Decode check v:'+streamComponent.isVideo+' bl:'+streamComponent.buffer.length+' ST:'+streamComponent.buffer[0].startTime);
		...
		xhr.onreadystatechange = function () {
			if (xhr.readyState == xhr.DONE) {
				...
				if (window.DOMParser) {
					...
					var xmlDoc = parser.parseFromString(xhr.response, "text/xml", 0);
				} else {
