...
- anything else you want passed on to the deliveryFunction call
	*/
	this.play = function(deliveryFunction,parameters) {
		...
		if (!parameters.format || parameters.allowedFormats.indexOf(parameters.format) == -1)
        ...
        // and also set as secure or not (default pic and http)
		...
		if (testVideo && testVideo.canPlayType) {
			var canEmbedHLS = testVideo.canPlayType('application/vnd.apple.mpegURL') !== '';
		}
		...
		if (unifiedPlayer.embedElementId == undefined || (parameters.format == 'HLS' && !canEmbedHLS)) {
			try {
				if ((parameters.format == 'HLS') && (navigator.userAgent.indexOf("iPhone") != -1) || (navigator.userAgent.indexOf("iPad") != -1)) {
					...
					var determineWidth = window.innerWidth ? window.innerWidth : document.documentElement.clientWidth ? document.documentElement.clientWidth : screen.width;
					var determineHeight = window.innerHeight ? window.innerHeight : document.documentElement.clientHeight ? document.documentElement.clientHeight : screen.height;
					var centerLeft = ((determineWidth / 2) - (winWidth / 2)) + dualScreenLeft;
					var centerTop = ((determineHeight / 2) - (winHeight / 2)) + dualScreenTop;
					...
					if (parameters.modal === undefined) {
						document.unifiedPlayerVideoWindow = window.open("", windowName,"modal=yes,resizable=1, top="+centerTop+", left="+centerLeft+", width="+winWidth+", height="+winHeight);
...
'<param name="initParams" value="initialStreamSource='+parameters.url+',isSmoothStream=true,initialPositionInSeconds='+parameters.startSeconds+'" />'
...
Please install the Silverlight plugin, or choose another format\
			...
			} else if (parameters.format == 'DASH') {
				container.innerHTML = "";
				player = new DashPlayer();
				if(player.isDASHCapable()) {
					var markup =  dashPlayerMarkup; //global set in dash_player_markup.js
					var dashControls = new dashVideoControls(container, markup,  parameters.embedId, parameters.movieId, player, parameters.url, parameters.trickPlayImgPrefix);
					unifiedPlayer._videoElement = document.querySelector('#' + dashControls.playerInstanceId + ' video');
					...
					//unifiedPlayer._videoElement.play(); // Android :/ //THIS WILL NOT WORK AS CURRENTLY IMPLEMENTED BY GOOGLE
					...
					//dashControls.failReason = 'This browser does not support Media Source Extensions<br>Please try a player format other than DASH';
				}
			} else if (parameters.format == 'MP4') {
				container.innerHTML = '';
				unifiedPlayer._videoElement = document.createElement('video');
				unifiedPlayer._videoElement.setAttribute('autoplay','autoplay');
...
//				innerDiv.appendChild(unifiedPlayer._videoElement);
			...
			if (parameters.format == 'HLS' || parameters.format == 'MP4') {
				if (document.unifiedPlayerVideoWindow)
...
//console.log('H264:'+window.MediaSource.isTypeSupported('video/mp4;codecs=avc1.42001f'));
		...
		if (allowedFormats.indexOf('DASH') != -1) {
			if (window.MediaSource && // browser must support MediaSourceExtensions
				window.MediaSource.isTypeSupported('video/mp4;codecs=avc1.42001f') && // requires basic h.264 decoder
				window.MediaSource.isTypeSupported('audio/mp4;codecs=mp4a.40.2') && // requires basic AAC decoder
		...
		if (allowedFormats.indexOf('HLS') != -1) {
		...
		if (allowedFormats.indexOf('FlashDynamic') != -1) {
		...
		if (allowedFormats.indexOf('SmoothStreaming') != -1) {
		...
		if (allowedFormats.indexOf('MP4') != -1) {
			return 'MP4';
		}
